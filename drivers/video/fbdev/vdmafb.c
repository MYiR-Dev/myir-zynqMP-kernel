	...
	if (prop)
	{
		if (strlen((char *)prop) <= VMODE_NAME_SZ)
		{
			...
			if (dn)
			{
				...
				strcpy(str, (char *)prop);
				c = strchr((char *)str, '_');
				if (c)
					*c = 0;
				strcpy(init_data->vmode_data.fb_vmode_name, (char *)str);
			}
			else
			{
...
void ClkWriteReg (ClkConfig *regValues, u64 dynClkAddr)
{
...
static int vdmafb_probe(struct platform_device *pdev)
{
	...
	//clkReg.clk0L = 0x400041;
	//clkReg.clkFBL = 0x69a;
	//clkReg.clkFBH_clk0H = 0x0;
	//clkReg.divclk = 0x20c4;
	//clkReg.lockL = 0xcfafa401;
	//clkReg.fltr_lockH = 0xa300ff;

	//writel(0x400041, ld->clk_base_virt + OFST_DYNCLK_CLK_L);
        //writel(0x69a, ld->clk_base_virt + OFST_DYNCLK_FB_L);
        //writel(0x0, ld->clk_base_virt + OFST_DYNCLK_FB_H_CLK_H);
        //writel(0x20c4, ld->clk_base_virt + OFST_DYNCLK_DIV);
        //writel(0xcfafa401, ld->clk_base_virt + OFST_DYNCLK_LOCK_L);
        //writel(0xa300ff, ld->clk_base_virt + OFST_DYNCLK_FLTR_LOCK_H);
